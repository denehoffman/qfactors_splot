...
def plot_events(
    ...
    nw, bw, _ = ax[0, 0].hist(
        ms, bins=100, range=(bounds["m_min"], bounds["m_max"]), weights=weights, label=weights_label, color=PALE_GRAY
    )
    ax[0, 0].hist(
        ms, bins=100, range=(bounds["m_min"], bounds["m_max"]), weights=weights, histtype="step", color=ERROR_RED
    )
    nt, bt, _ = ax[0, 0].hist(
        ms_sig, bins=100, range=(bounds["m_min"], bounds["m_max"]), histtype="step", label="Truth", color="black"
    )
    ...
    nw, bw, _ = ax[1, 0].hist(
        ts, bins=100, range=(bounds["t_min"], bounds["t_max"]), weights=weights, label=weights_label, color=PALE_GRAY
    )
    ax[1, 0].hist(
        ts, bins=100, range=(bounds["t_min"], bounds["t_max"]), weights=weights, histtype="step", color=ERROR_RED
    )
    nt, bt, _ = ax[1, 0].hist(
        ts_sig, bins=100, range=(bounds["t_min"], bounds["t_max"]), histtype="step", label="Truth", color="black"
    )
    ...
    nw, bw, _ = ax[1, 1].hist(
        gs, bins=100, range=(bounds["g_min"], bounds["g_max"]), weights=weights, label=weights_label, color=PALE_GRAY
    )
    ax[1, 1].hist(
        gs, bins=100, range=(bounds["g_min"], bounds["g_max"]), weights=weights, histtype="step", color=ERROR_RED
    )
    nt, bt, _ = ax[1, 1].hist(
        gs_sig, bins=100, range=(bounds["g_min"], bounds["g_max"]), histtype="step", label="Truth", color="black"
    )
...
def plot_all_events(events_sig: list[Event], events_bkg: list[Event], filename="generated_data.png", directory="study"):
    ...
    # signal plots
    ax[0, 0].hist(ms_sig, bins=100, range=(bounds["m_min"], bounds["m_max"]), label="signal", color=BLUE)
    ax[0, 0].set_xlabel(r"$M_{3\pi}$ (GeV/$c^2$)")
    ax[0, 0].set_ylabel(r"Counts / 0.002")
    ax[0, 0].legend(loc="upper right")
    ...
    ax[0, 1].set_xlabel(r"$\cos(\theta)$")
    ax[0, 1].set_ylabel(r"$\phi$")
    ax[0, 2].hist(ts_sig, bins=100, range=(bounds["t_min"], bounds["t_max"]), label="signal", color=BLUE)
    ax[0, 2].set_xlabel("$t$ (arb)")
    ax[0, 2].set_ylabel(r"Counts / 0.02")
    ax[0, 2].legend(loc="upper right")
    ax[0, 3].hist(gs_sig, bins=100, range=(bounds["g_min"], bounds["g_max"]), label="signal", color=BLUE)
    ax[0, 3].set_xlabel("$g$ (arb)")
    ax[0, 3].set_ylabel(r"Counts / 0.036")
    ax[0, 3].legend(loc="upper right")

    # background plots
    ax[1, 0].hist(ms_bkg, bins=100, range=(bounds["m_min"], bounds["m_max"]), label="background", color=RED)
    ...
    ax[1, 2].hist(ts_bkg, bins=100, range=(bounds["t_min"], bounds["t_max"]), label="background", color=RED)
    ax[1, 2].set_xlabel("$t$ (arb)")
    ax[1, 2].set_ylabel(r"Counts / 0.02")
    ax[1, 2].legend(loc="upper right")
    ax[1, 3].hist(gs_bkg, bins=100, range=(bounds["g_min"], bounds["g_max"]), label="background", color=RED)
...
def plot_qfactor_fit(mstar, ms, z_fit: float, b_fit: float, event_index: int, qfactor_type: str, directory="study"):
    ...
    plt.plot(
        m_range, np.array([z_fit * m_sig(m_val) for m_val in m_range]), ls="-", lw=2, color=BLUE, label="Signal Fit"
    )
    plt.plot(
        m_range,
